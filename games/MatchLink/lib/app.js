$(document).ready(function(){
	...
	var endlessTime = 0;	//Set counter for endless difficulty
	...
	function cellSpawn(interval)
	{
		setInterval(function(){
			var location1 = getRandomArbitrary(0, emptyCells.length);
			 var location2 = getRandomArbitrary(0, emptyCells.length/2);
			 emptyCells.each(function(i){
			 	if(i == location || i == location2)
			 	{
			 		var newType = cellTypes[Math.floor(Math.random()*cellTypes.length)];
			 		emptyCells[i].addClass(newType);
			 		activeCells.push(emptyCells[i]);
	...
	function spawnTimer(array)
	{
		setInterval(function(){
			endlessTime = endlessTime + 1;
			cellSpawn(6000 / endlessTime);
		}, 15000);
	}
